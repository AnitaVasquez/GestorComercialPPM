...
{
    [Autenticado]
    public class MatrizPresupuestoController : Controller
    {
        ...
        {
            ViewBag.TituloModal = "Reporte Matriz de Presupuesto";
            return PartialView();
        }
        ...
        public ActionResult ReporteMatriz(int id, string tituloReporte, string ejecutivo, DateTime fechaInicio, DateTime fechaFin)
        {
            try
            {
                ...
                var codigoCotizacion = CodigoCotizacionEntity.ConsultarCodigoCotizacionCompleto(id);

                string codigoCotizacion2 = codigoCotizacion != null ? codigoCotizacion.codigo_cotizacion : "SinCodigo" + Guid.NewGuid().ToString().Substring(0, 6);
                ...
                var ws = package.Workbook.Worksheets.Add(codigoCotizacion2);
                ...
                List<MatrizPresupuestoInfo> matriz = MatrizPresupuestoEntity.ConsultarMatrizPresupuestoByCodigoCotizacion(id, fechaInicio, fechaFin);

                var columnas = new List<string> { "N°", "CÓDIGO DE COTIZACIÓN", "N. DOCUMENTO", "TIPO DE PRODUCTO Y/O SERVICIO", "DETALLE", "VALOR", "IVA", "TOTAL" };
                        ...
                        for (int j = 1; j <= 8; j++)
                        {
                            ...
                            switch (j)
                            {
                                case 1:
                                    ...
                                    break;
                                case 2:
                                    ws.Cells[finCabecera, j].Value = item.codigo_cotizacion;
                                    break;
                                case 3:
                                    ...
                                    break;
                                case 4:
                                    ws.Cells[finCabecera, j].Value = item.ProductoServicio;
                                    ws.Cells[finCabecera, j].Style.WrapText = true;
                                    break;
                                case 5:
                ...
                string rutaArchivos = basePath + "\\GESTION_PPM\\MATRIZ_PRESUPUESTO";


                var anioActual = DateTime.Now.Year.ToString();
                var almacenFisicoTemporal = Auxiliares.CrearCarpetasDirectorio(rutaArchivos, new List<string>() { anioActual, codigoCotizacion2 });

                //var almacenFisicoOfficeToPDF = Auxiliares.CrearCarpetasDirectorio(Server.MapPath("~/OfficeToPDF/"), new List<string>());

                // Get the complete folder path and store the file inside it.    
                string pathExcel = Path.Combine(almacenFisicoTemporal, "MatrizPresupuesto" + codigoCotizacion2 + ".xlsx");
