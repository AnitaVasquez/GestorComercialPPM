...
(function($) {
            ...
            if ($childNodes.length > 0) {
                ...
                $nodeDiv.find(".exp-col").click(function() {
                    ...
                    if ($tr.hasClass('contracted')) {
                        ...
                        $tr.nextAll("tr").css('visibility', 'visible').fadeIn(400);
                        $node.removeClass('collapsed');
                    } else {
                        $tr.removeClass('expanded').addClass('contracted');
                        $tr.nextAll("tr").css('visibility', 'hidden').fadeOut(400);
        ...
        if (opts.control) {
            if (!$nodeDiv.hasClass("temp")) {
                ...
                if ($nodeDiv.hasClass("child")) {
                    $nodeDiv.find(".opciones:eq(0)").append("<span title='Eliminar elemento.' class='del'></span>");
                }
            } else {
    ...
    function remChild(removing) {
        debugger
        $(removing).find('a').css({ 'font-size': "11px" });
        $(removing).find('.cargo').css({ 'font-size': "9px" });
        $(removing).css({ 'cursor': "grab" });
        //$(removing).find("i")[1].remove();

        //var cargo = $(removing).find('i').text();

        //var tagCargo = '<small><b style="font-size: 9px;"><i class="fa fa-briefcase" aria-hidden="true"></i>' + cargo +'</b></small>'

        
        //$(removing).find(".cargo").remove();
        

        //$(removing).append(tagCargo);
